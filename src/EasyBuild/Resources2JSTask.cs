...
namespace EasyBuild
{
    ...
    public class Resources2JSTask : Task
    {
            ...
            using (var fileStream = new StreamWriter(File.Open(serializationFilename, FileMode.Create, FileAccess.Write), Encoding.UTF8))
            {
                foreach (var rt in m_resourceTypes)
                {
                    ...
                    for (int i = 0; i < properties.Count; i++)
                    {
                        ...
                        fileStream.Write("  '{0}': '{1}'", NormalizePropertyName(p.Name), NormalizePropertyValue(globalizedText));
        ...
        private static string NormalizePropertyValue(string globalizedText)
        {
